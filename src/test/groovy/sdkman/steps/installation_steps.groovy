...
import static bliman.support.FilesystemUtils.prepareCandidateBinFolder
import static bliman.support.FilesystemUtils.prepareCandidateWithVersionFolder
...
And(~'^I have a local candidate "([^"]*)" version "([^"]*)" at relative path "([^"]*)"$') { String candidate, String version, String relativePath ->
	def fullPath = new File(blimanBaseDir.absolutePath, relativePath)
...
And(~'^I have configured "([^"]*)" to "([^"]*)"$') { String configName, String flag ->
	def configFile = new File("$blimanDir/etc/config")
