...
import static bliman.stubs.HookResponses.*
import static bliman.stubs.WebServiceStub.*
import static bliman.support.FilesystemUtils.readCurrentFromCandidateFolder
import static bliman.support.FilesystemUtils.readVersionsCsvFromCandidateFolder
...
And(~'^an available selfupdate endpoint$') { ->
	primeEndpointWithString("/selfupdate/stable/${UnixUtils.inferPlatform()}", 'echo "Successfully upgraded BLIMAN."')
	primeEndpointWithString("/selfupdate/beta/${UnixUtils.inferPlatform()}", 'echo "Successfully upgraded BLIMAN."')
}
...
And(~'^the candidate "([^"]*)" version "([^"]*)" is available for download with checksum "([^"]*)" using algorithm "([^"]*)"$') { 
	...
	primeEndpointWithString("/candidates/validate/${candidate}/${version}/${UnixUtils.inferPlatform()}", "valid")
	primeDownloadFor(SERVICE_UP_URL, candidate, version, UnixUtils.inferPlatform(), ["X-Bliman-Checksum-${algorithm}": "${checksum}"])
	primeEndpointWithString("/hooks/post/${candidate}/${version}/${UnixUtils.inferPlatform()}", postInstallationHookSuccess())
}
